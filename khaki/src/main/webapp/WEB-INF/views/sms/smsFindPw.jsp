...
<%@page import="co.kr.khaki.member.MemberDTO"%>
   ...
   <!--
    ...
    <form method="post" name="smsForm" id="smsForm" action="smssend_findIdPw.do">   <!-- action에 발송처리를 담당할 jsp파일을 입력해줌  -->
        ...
        <input type="hidden" name="subject" value="KHAKI 임시 비밀번호 발급" id="subject"><!--  장문(LMS)인 경우(한글30자이내) -->
        
        <!-- textarea -->
 
        <textarea name="msg" cols="30" rows="10" style="width:455px; display: none;" id="content" >[KHAKI] <%=memberDTO.getName() %> 회원님의 계정, <%=memberDTO.getId() %>의 임시 비밀번호는
        [ <%=memberDTO.getPw() %> ]입니다.
